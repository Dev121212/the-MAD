...
class Home extends React.Component {
  ...
  handleGoogleLogin = () => {
    ...
    firebase.auth().signInWithPopup(provider).then(async function (result) {
      ...
      var credential = await firebase.auth.GoogleAuthProvider.credential(result.getAuthResponse().id_token);
  ...
  render() {
    return (
      <div>
        <button onClick={this.signInAnonymous} disabled={this.state.showGoogleButton}>
          Start the Game
        </button>
        {/* <DashboardContainer /> */}
        <h1>
          is {this.state.isAnonymous}
        </h1>
        <h1>
          {this.state.uid}
        </h1>
